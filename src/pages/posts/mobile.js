export const mobile = (
    <>
        ...
        <div className='text'>
            ...
            <p>When coding a new page or component I will view the page in a small, skinny window as if it were someone's device. Specifically, I use Chrome's inspect tool (right-click the browser page, select Inspect, then click the mobile button towards the top). I tend to select one of the iPhone options, both because this is a popular choice of phone owners and because some of the iPhones are pretty narrow and I want to ensure all my content will fit. </p>
            <p>Once I have finalized the html/css for mobile, only then will I exit out of the mobile-view and see what the current code looks like on a bigger screen. (Sometimes that can be quite a surprise.) If the larger screen needs some adjustments, I will add media queries specifying a minimum screen size.</p>
            ...
            <p>There are some interesting statistics about the growth of smart phone users over the past few years, but the number that really sticks out to me is that, according to the Pew Research Center<sup>1</sup>, 15% of American adults are “smartphone-only” internet users, meaning they will only ever see this site from a mobile view. </p>
            <p>I feel like there is also a convenience factor with smart phones, if I were to text a link of this site to a friend or share it on social media, the recipient would almost certainly open the site on their phone first and I would want the site to make a good first impression. </p>
            <p>From a practical standpoint, it is a lot easier to expand a small design out onto a larger screen than it is to cram a large design into a much smaller space. In the past, I would code using my full browser page and found that it could be really hard to fit readable text or pictures onto a smaller screen when making the project mobile-friendly was an afterthought.</p>
            <p>Working mobile-first ensures that phone users receive a full-fledged product, not just a watered-down version of the desktop version.</p>
            <p>I have also found that working mobile-first leads me towards using code solutions that work across a variety of screen sizes such as an emphasis on flexbox or using a hamburger menu in the desktop view.</p>
            <hr />
